...
export const multicaLocalAgentWorkflow = {
  ...
  content: [
    ...
    '它好用的地方还在于 runtime 可以贴近真实环境。最简单的模式是本地：本地仓库、本地 CLI、本地 API key、本地测试环境，都在自己机器上跑。更进一步，Multica 也可以连接远程机器，把 task delegate 给远程 agent。这样一来，它原理上不只适合写代码，也能覆盖一些运维和排障场景。',
    'AI 在运维侧其实非常强。它能快速读日志、串联系统状态、组合诊断命令，有时候能打出人类很难一次性敲出来的长命令。但越是这样，越不能把它当成无边界的自动运维。远程机器、生产环境、密钥、sudo 权限、删除和重启类操作都要尤其小心，权限管理和审批边界必须提前设计好。',
    '我更愿意把它先当成一个很顺手的问题定位工具：让 agent 去收集上下文、整理日志、提出判断、生成候选命令，人再决定哪些命令可以执行。这样既能吃到 AI 在运维排障上的效率，也不会把关键系统完全交给一个自动执行链。',
