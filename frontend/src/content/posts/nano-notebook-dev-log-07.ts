...
export const nanoNotebookDevLog07 = {
  ...
  content: [
    ...
    '最直接的写法是把 system prompt 写成 Go 里的一个字符串常量，或者在代码里拼一段 Markdown。单个 demo 没问题，一旦 Agent 上线，它会立刻带来三件事：',
    ...
    'nano-notebook 把 Agent 行为拆成文件，每个文件都有 `identity + version`，再让运行数据引用版本而非文本。',
    ...
    'Catalog 加载时用 strict JSON decode，unknown field 直接失败；所有 reference 必须解析到已存在版本；文件名也必须和 `identity + version` 一致。这样“prompt 版本”是 Agent Definition 的一个坐标。`chat.leader@3` 要复现，就包含这三个 prompt 版本、model policy、工具 allowlist、子 Agent 和预算。改工具或改 prompt 都会产生新的 definition version / release，而不是在代码里静默覆盖。',
    '## 记录版本之后，Replay 才有坐标',
    ...
    '这也改变 Replay 的用法：先精确区分“这次变化影响谁”，再用固定 dataset 做回归。旧版本不会因为新版本上线就丢失身份，线上所有用旧 prompt 的 Run 仍然能按 hash 查询和回放。',
    '## 几个边界',
    ...
    'prompt 版本控制让 Agent 的每个行为输入都有 identity、hash 和引用关系。Run 只要记录这些引用，前面所有 Trace/Replay 能力就都能回答同一个问题：这个结果是在哪个 Agent 定义下产生的。',
    '## 源码锚点',
