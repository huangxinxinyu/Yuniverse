...
export const nanoNotebookDevLog02 = {
  ...
  content: [
    ...
    '关键是“什么状态是权威的、谁有权继续执行、谁有权发布结果”。',
    '## 这个目标会怎么影响技术选型',
    '如果把 Agent 循环只放在进程内存里，模型调用、工具调用和中间 Draft 都会跟着进程消失。即使一个框架提供了 in-memory loop，也只是把这个问题藏起来，没有真正解决。',
    '要让可恢复和可中断成为产品承诺，就必须拥有 Durable Run、Durable Job、Lease Fencing、Checkpoint、Cancellation 和 Publication Barrier。技术选型的第一问是“这个方案能不能让我拥有这些状态”。',
    '## 我们比较的四类方案',
    ...
    '这些框架都默认 Host 负责 Durable State；而 Durable State 恰好是我们这个项目的核心目标。所以自研把本来就必须自己拥有的 Runtime 直接做成第一公民。',
    '## 自研 Runtime 的骨架',
    'nano-notebook 的自研 Runtime 刻意做得很窄，是一套固定产品 Job + Agent Run 状态机。核心链路是：',
    ...
    '模型决策、Action 和最终 Draft 都以 Append-Only Checkpoint 落库。每个已接受边界有唯一 Identity，恢复时从第一个不完整步骤继续，已接受的 Proposal/Result 不会被重复计费。最终答案通过 Publication Barrier 重新校验用户、权限、Source、Cancellation、Lease Token 和 Citation 后，才写进 Chat 成为正式消息。',
    '## 崩溃恢复和用户取消不是同一件事',
    ...
    '模型层通过 Bifrost Gateway 做协议归一化和 bounded retry，不把 Provider SDK 直接长进 Worker。Runtime 关心模型决策和工具调用边界，不关心某个 Provider 的内部状态；换模型不会改变 Job、Lease、Checkpoint 和发布语义。',
    ...
    '## 这篇没有展开什么',
    '这篇只讲 Runtime 骨架。RAG、Durable Trace、MCP Tool Plane、权限、并发工具调用会留给后续日志；它们在仓库里都有真实实现和测试。',
